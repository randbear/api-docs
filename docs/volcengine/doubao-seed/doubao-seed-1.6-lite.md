---
title: doubao-seed-1.6-lite 对话模型
provider: volcengine
model_id: doubao-seed-1-6-lite-251015
task: chat
---

# doubao-seed-1.6-lite 对话模型

> 豆包 seed 1.6 轻量版模型，成本更低，支持深度思考和多模态理解，适合日常对话和轻量任务。

## 模型信息

| 属性 | 值 |
|------|-----|
| 模型ID | `doubao-seed-1-6-lite-251015` |
| 任务类型 | 对话（Chat） |
| 输入 | 文本 / 图片 / 视频 |
| 输出 | 文本 |
| 上下文窗口 | 256k tokens |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| model | string | 是 | — | `doubao-seed-1-6-lite-251015` |
...
按 token 用量计费，单位：元/百万 token。价格根据输入长度和输出长度分层：

| 输入长度（千 token） | 条件 | 输入单价 | 输出单价 |
|----------------------|------|----------|----------|
| [0, 32] | 输出长度 [0, 0.2]k | 0.30 | 0.60 |
| [0, 32] | 输出长度 (0.2, +∞)k | 0.30 | 2.40 |
| (32, 128] | — | 0.60 | 4.00 |
| (128, 256] | — | 1.20 | 12.00 |
...
| 特性 | seed-1.6 | seed-1.6-lite | seed-1.6-flash | seed-1.6-vision |
|------|----------|---------------|----------------|-----------------|
| 输入 [0,32]k | 0.80 | **0.30** | 0.15 | 0.80 |
| 输出 (短) | 2.00 | **0.60** | 1.50 | 8.00 |
| 结构化输出 | 是 | 否 | 是 | 是 |
| 视觉定位 | 否 | 否 | 是 | 否 |
| GUI 任务 | 否 | 否 | 否 | 是 |
